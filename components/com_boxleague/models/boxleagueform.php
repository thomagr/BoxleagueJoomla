 ...
 * @since  1.6
 */
class BoxleagueModelBoxleagueForm extends \Joomla\CMS\MVC\Model\FormModel
{
    private $item = null;
    ...
    protected function populateState()
    {
        ...
        {
                $id = Factory::getApplication()->getUserState('com_boxleague.edit.boxleague.id');
        }
        else
        {
                $id = Factory::getApplication()->input->get('id');
                Factory::getApplication()->setUserState('com_boxleague.edit.boxleague.id', $id);
        }

        $this->setState('boxleague.id', $id);
        ...
        {
                $this->setState('boxleague.id', $params_array['item_id']);
        }
    ...
    {
        // Get the id.
        $id = (!empty($id)) ? $id : (int) $this->getState('boxleague.id');
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_boxleague.boxleague', 'boxleagueform', array(
                        'control'   => 'jform',
                        'load_data' => $loadData
                )
        );
    ...
    {
        $id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('boxleague.id');
    ...
    public function delete($pk)
    {
        $user = Factory::getUser();
            ...
            if (empty($pk))
            {
                    $pk = (int) $this->getState('boxleague.id');
            }
